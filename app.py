...
# ---------------------------------------
# 0. 시스템 설정: Veritas Clinical Engine v4.4 (Stable Stream)
# ---------------------------------------
st.set_page_config(
    page_title="Veritas Clinical Engine v4.4 | 자연과한의원",
    page_icon="🧬",
    layout="centered"
)
...
# [수정됨] 마크다운 깨짐 방지를 위해 커서(●) 제거 및 로직 단순화
def claude_stream(text, speed=0.01):
    placeholder = st.empty()
    display_text = ""
    # 텍스트를 3글자씩 덩어리로 출력하여 끊김 완화
    chunk_size = 3 
    for i in range(0, len(text), chunk_size):
        chunk = text[i:i+chunk_size]
        display_text += chunk
        placeholder.markdown(display_text) # 커서 제거
        time.sleep(speed)
    # 최종 출력 보장
...
# ---------------------------------------
st.markdown("<h3 style='margin-bottom:0; font-family: serif; color: white;'>Veritas Clinical Engine v4.4</h3>", unsafe_allow_html=True)
